"""Sing-along player: plays the song and highlights the lyric line being sung and the source words it sings."""
...
function render({ model, el }) {
  ...
  function build() {
    ...
    // The source paragraph itself, word by word.
    const p = document.createElement("p");
    p.className = "sa-karaoke";
    (model.get("source_words") || []).forEach((w, i) => {
  ...
  function highlight() {
    ...
    let first = null;
    sourceEl.querySelectorAll(".sa-karaoke span").forEach((el) => {
...
.sa-source p.sa-karaoke { margin: 0; padding: 6px 8px; font-size: 15px; line-height: 1.8; }
...
def from_result(done_event: dict, setup_event: dict, mp3_path: Path) -> SingAlong:
    ...
    return SingAlong(audio_src=audio, topic=setup_event["topic"], url=setup_event.get("url", ""), lines=lines,

...
def load_profile(transcription: Path = TRANSCRIPTION) -> dict:
    ...
    return {"header": header, "sections": sections, "transcription": str(transcription)}
...
def source_ranges(sections: list[Section]) -> list[tuple[float, float]]:
    return [(s.start, s.end) for s in sections]


def count_text_syllables(text: str) -> int:
    from faithful import tokens  # spells out digits
    return sum(pipeline.count_syllables(w) for w in tokens(text))
